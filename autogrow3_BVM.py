...
class Molecule2(pymolecule.Molecule):
    ...
    def Distanc(self):
        ...
        count = 0
        for index in self.all_atoms:
            if self.all_atoms[index].chain == "G" :
                x1 =  self.all_atoms[index].coordinates.x
                y1 =  self.all_atoms[index].coordinates.y
                z1 =  self.all_atoms[index].coordinates.z
            elif self.all_atoms[index].chain == "D":
                x2 =  self.all_atoms[index].coordinates.x
                y2 =  self.all_atoms[index].coordinates.y
                z2 =  self.all_atoms[index].coordinates.z
        DeltaX = x1 - x2;
        DeltaY = y1 - y2;
        DeltaZ = z1 - z2;
        distance = numpy.sqrt(DeltaX*DeltaX+DeltaY*DeltaY+DeltaZ*DeltaZ)
...
class run_autoclick_multithread(general_task):
    ...
    def value_func(self, command, variables_to_pass):
        ...
        #jobs.append(vars['babel_executable'] + ' -ipdb ' + fragments + ' -opdb ' + fragments + ' h ')
        ...
        #'obabel' + ' -ipdb ' + fragment + ' -opdb -O ' + fragment + ' -h '
        #jobs.append('obabel' + ' -ipdb ' + fragment + ' -opdb -O ' + fragment + ' -h ')
        #multi_threading(jobs, execute_command, {'seconds_per_job': 10})
        #print(fragment)
...
def check_drug_likeness(pdb_filename):
    ...
    if lipinski == None or lipinski_strict == None or ghose == None:
        ...
        if not os.path.exists(pdb_basename + ".info"): 
            os.popen(vars['obprop_executable'] + " " + pdb_filename + " > " + pdb_basename + ".info 2>&1", 'r')
        logP = None
        # os.system("ls autogrow_output/generation1/support/")
...
def convert_ligand_pdb_files_to_pdbqt(directory):
    ...
    # Remove IP5 hydrogens
    os.mkdir("tmp")
    jobVMD = []
    jobMIN = []
    for lig_filename in glob.glob(directory + "*.pdb"):
            torun = ' vmd ' +  lig_filename + ' -dispdev ' + ' text ' + ' -e ' + 'Hremove.vmd ' 
            jobVMD.append(torun)
            
    multi_threading(jobVMD, execute_command, {'seconds_per_job': 100000})     
    
    # Merging the first two lines of the old version of pdb with the new one from vmd
    jobs = [];
    for lig_filename in glob.glob(directory + "*.pdb"):
        FileName = os.path.basename(lig_filename)
        jobs.append('head -n 2 ' + lig_filename + ' | tee ' + lig_filename + ' && tail -n +2 ' + ' tmp/' + FileName + ' | tee -a ' +  lig_filename)
    multi_threading(jobs, execute_command, {'seconds_per_job': 1000}) 
    
    # Copy the newly generated file to the origin folder
    jobMIN = []
    for lig_filename in glob.glob(directory + "*.pdb"):
            FileName = os.path.basename(lig_filename)
            if not os.path.exists(directory + "origin" + os.sep): os.makedirs(directory + "origin" + os.sep)
            Minimiz = 'obminimize -ff GAFF -n 5000 ' + lig_filename  + ' > ' + directory + 'origin/' + FileName
            jobMIN.append(Minimiz)
            
    multi_threading(jobMIN, execute_command, {'seconds_per_job': 100000}) 
    shutil.rmtree('tmp')
    sys.exit(1)
    ...
    for line in f.readlines():
        ...
        if line[:5] == "ATOM " or line[:7] == "HETATM ":
            ...
            for i in range(len(middle_lastpart)):
                if middle_lastpart[:1] in string.ascii_uppercase:
...
def dock_compounds(directory):
    ...
    for receptor_filename in receptors:
        for lig_filename in need_to_dock:
            torun = vars['vina_executable'] + " --center_x " + str(vars['center_x']) + " --center_y " + str(vars['center_y']) + " --center_z " + str(vars['center_z']) + " --size_x " + str(vars['size_x']) + " --size_y " + str(vars['size_y']) + " --size_z " + str(vars['size_z']) + " --receptor " + receptor_filename + " --ligand " + lig_filename + " --out " + lig_filename + ".vina --cpu 96" + " --exhaustiveness 10 " + " --num_modes 3 "
...
def delete_bad_ligands(current_generation_dir):
    ...
    for filename in glob.glob(current_generation_dir + "*.pdb"):
        ...
        tmp_f = open(filename,'r')
        pdb = Molecule2()
        pdb.load_pdb(filename)
        distance = pdb.Distanc()
        if (distance < 3.5 or distance > 9.5):
            delete_all_associated_files(filename)
